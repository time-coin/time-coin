...
fn test_vote_approval_scenarios() {
    ...
    let approvals = 2;
    let required = (total_nodes * 2).div_ceil(3);
    assert!(approvals >= required, "Should reach consensus with 2/3 approvals");
    
    // Scenario 2: 3 nodes, 1 approves (consensus NOT reached)
    let approvals = 1;
    assert!(approvals < required, "Should NOT reach consensus with 1/3 approvals");
    
    // Scenario 3: 6 nodes, 4 approve (consensus reached)
    let total_nodes: usize = 6;
    let approvals = 4;
    let required = (total_nodes * 2).div_ceil(3);
    assert!(approvals >= required, "Should reach consensus with 4/6 approvals");
    
    // Scenario 4: 6 nodes, 3 approve (consensus NOT reached)
    let approvals = 3;
    assert!(approvals < required, "Should NOT reach consensus with 3/6 approvals");
}
...
fn test_vote_rejection_scenarios() {
    ...
    let required = (total_nodes * 2).div_ceil(3);
    assert!(approvals >= required, "Should reach consensus despite rejection");
}
...
fn test_partial_response_scenarios() {
    ...
    assert!(approvals >= required, "Should reach consensus with 3/4 responding");
}
...
fn test_timeout_handling() {
    ...
    assert!(vote_timeout_secs >= 3, "Timeout should be at least 3 seconds");
    
    // But not too long to delay finality
    assert!(vote_timeout_secs <= 10, "Timeout should not exceed 10 seconds");
}
...
fn test_dev_mode_auto_finalization() {
    ...
    if connected_peers == 0 && is_dev_mode {
        // Should auto-finalize in dev mode
        assert!(true, "Dev mode should auto-finalize with no peers");
    }
}

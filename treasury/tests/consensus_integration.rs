...
fn test_scenario_5_insufficient_funds_validation() {
    ...
    match schedule_result {
        Err(TreasuryError::InsufficientBalance { requested, available }) => {
...
fn test_one_vote_per_masternode_validation() {
    ...
    assert!(result2.is_err(), "Second vote from same masternode should fail");
...
fn test_masternode_tier_weights() {
    ...
    assert!(proposal.has_approval(), "Should be approved with 100% YES votes");
}
...
fn test_voting_on_non_active_proposal_rejected() {
    ...
    assert!(result.is_err(), "Vote on non-active proposal should be rejected");
    assert_eq!(proposal.votes.len(), 0, "No votes should be recorded");
}

...
fn test_e2e_complete_proposal_lifecycle_mixed_tiers() {
    ...
    assert_eq!(total_power, 355, "Network should have 355 total voting power");
    
    // Phase 1: Fund the treasury
    let mut treasury = TreasuryPool::new();
    for block in 1..=500 {
        treasury.deposit_block_reward(block, 1000 + block)
    ...
    proposal.add_vote("gold-node-1".to_string(), VoteChoice::Yes, 100, 10100)
        .expect("Gold-1 vote failed");
    proposal.add_vote("gold-node-2".to_string(), VoteChoice::Yes, 100, 10200)
        .expect("Gold-2 vote failed");
    proposal.add_vote("gold-node-3".to_string(), VoteChoice::Yes, 100, 10300)
        .expect("Gold-3 vote failed");
    
    // 3 Silver nodes vote YES, 2 vote NO (30 YES, 20 NO)
    proposal.add_vote("silver-node-1".to_string(), VoteChoice::Yes, 10, 10400)
        .expect("Silver-1 vote failed");
    proposal.add_vote("silver-node-2".to_string(), VoteChoice::Yes, 10, 10500)
        .expect("Silver-2 vote failed");
    proposal.add_vote("silver-node-3".to_string(), VoteChoice::Yes, 10, 10600)
        .expect("Silver-3 vote failed");
    proposal.add_vote("silver-node-4".to_string(), VoteChoice::No, 10, 10700)
        .expect("Silver-4 vote failed");
    proposal.add_vote("silver-node-5".to_string(), VoteChoice::No, 10, 10800)
        .expect("Silver-5 vote failed");
    
    // 3 Bronze vote YES, 1 votes NO, 1 abstains
    proposal.add_vote("bronze-node-1".to_string(), VoteChoice::Yes, 1, 10900)
        .expect("Bronze-1 vote failed");
    proposal.add_vote("bronze-node-2".to_string(), VoteChoice::Yes, 1, 11000)
        .expect("Bronze-2 vote failed");
    proposal.add_vote("bronze-node-3".to_string(), VoteChoice::Yes, 1, 11100)
        .expect("Bronze-3 vote failed");
    proposal.add_vote("bronze-node-4".to_string(), VoteChoice::No, 1, 11200)
        .expect("Bronze-4 vote failed");
    proposal.add_vote("bronze-node-5".to_string(), VoteChoice::Abstain, 1, 11300)
    ...
    treasury.schedule_withdrawal(withdrawal)
        .expect("Failed to schedule withdrawal");
    
    let withdrawn = treasury.execute_withdrawal("withdrawal-e2e-001", after_deadline + 100)
        .expect("Failed to execute withdrawal");
    
    assert_eq!(withdrawn, 1_000 * TIME_UNIT);
    assert_eq!(treasury.balance_time(), 1500.0, "Treasury should have 1,500 TIME remaining");
    
    // Phase 7: Mark proposal as executed
    proposal.mark_executed().expect("Failed to mark as executed");
...
fn test_e2e_multiple_concurrent_proposals() {
    ...
    proposal_a.add_vote("gold-1".to_string(), VoteChoice::Yes, 100, 2000).unwrap();
    proposal_a.add_vote("gold-2".to_string(), VoteChoice::Yes, 100, 2100).unwrap();
    proposal_a.add_vote("silver-1".to_string(), VoteChoice::Yes, 10, 2200).unwrap();
    proposal_a.add_vote("silver-2".to_string(), VoteChoice::No, 10, 2300).unwrap();
    proposal_a.add_vote("bronze-1".to_string(), VoteChoice::No, 1, 2400).unwrap();
    
    // Voting on Proposal B: Strong NO (1 Gold YES, rest NO = 100 YES, 121 NO)
    proposal_b.add_vote("gold-1".to_string(), VoteChoice::Yes, 100, 2000).unwrap();
    proposal_b.add_vote("gold-2".to_string(), VoteChoice::No, 100, 2100).unwrap();
    proposal_b.add_vote("silver-1".to_string(), VoteChoice::No, 10, 2200).unwrap();
    proposal_b.add_vote("silver-2".to_string(), VoteChoice::No, 10, 2300).unwrap();
    proposal_b.add_vote("bronze-1".to_string(), VoteChoice::No, 1, 2400).unwrap();
    
    // Voting on Proposal C: Exactly at threshold (148 YES, 73 NO = 67%)
    // Need 67% of 221 = ~148.07
    proposal_c.add_vote("gold-1".to_string(), VoteChoice::Yes, 100, 2000).unwrap();
    proposal_c.add_vote("gold-2".to_string(), VoteChoice::No, 100, 2100).unwrap();
    proposal_c.add_vote("silver-1".to_string(), VoteChoice::Yes, 10, 2200).unwrap();
    proposal_c.add_vote("silver-2".to_string(), VoteChoice::No, 10, 2300).unwrap();
    ...
    assert!(proposal_a.has_approval(), "Proposal A should pass with 95% YES");
    
    let results_b = proposal_b.calculate_results();
    assert_eq!(results_b.yes_power, 100);
    assert_eq!(results_b.no_power, 121);
    assert!(!proposal_b.has_approval(), "Proposal B should fail with 45% YES");
    ...
    assert!(!proposal_c.has_approval(), "Proposal C should fail with 50% YES");
...
fn test_e2e_inactive_masternodes_excluded_from_voting() {
    ...
    // Active nodes vote
    proposal.add_vote("gold-active".to_string(), VoteChoice::Yes, 100, 2000).unwrap();
    proposal.add_vote("silver-active".to_string(), VoteChoice::Yes, 10, 2100).unwrap();
    proposal.add_vote("bronze-active".to_string(), VoteChoice::No, 1, 2200).unwrap();
...
fn test_e2e_milestone_based_proposal_execution() {
    ...
    proposal.add_vote("gold-1".to_string(), VoteChoice::Yes, 100, 2000).unwrap();
    proposal.add_vote("gold-2".to_string(), VoteChoice::Yes, 100, 2100).unwrap();
    proposal.add_vote("gold-3".to_string(), VoteChoice::Yes, 100, 2200).unwrap();
    ...
    treasury.execute_withdrawal("milestone-1", after_deadline + 100).unwrap();
    ...
    treasury.execute_withdrawal("milestone-2", after_deadline + 86400 * 30 + 100).unwrap();
    ...
    treasury.execute_withdrawal("milestone-3", after_deadline + 86400 * 60 + 100).unwrap();
...
fn test_e2e_varying_participation_rates() {
    let network = MasternodeNetwork::new(vec![
        Masternode::gold("g1"), Masternode::gold("g2"),
        Masternode::silver("s1"), Masternode::silver("s2"),
        Masternode::bronze("b1"), Masternode::bronze("b2"),
    ]);
    ...
    proposal_high.add_vote("g1".to_string(), VoteChoice::Yes, 100, 2000).unwrap();
    proposal_high.add_vote("g2".to_string(), VoteChoice::Yes, 100, 2100).unwrap();
    proposal_high.add_vote("s1".to_string(), VoteChoice::No, 10, 2200).unwrap();
    proposal_high.add_vote("s2".to_string(), VoteChoice::No, 10, 2300).unwrap();
    proposal_high.add_vote("b1".to_string(), VoteChoice::No, 1, 2400).unwrap();
    proposal_high.add_vote("b2".to_string(), VoteChoice::No, 1, 2500).unwrap();
    ...
    proposal_low.add_vote("g1".to_string(), VoteChoice::Yes, 100, 2000).unwrap();
    proposal_low.add_vote("g2".to_string(), VoteChoice::Yes, 100, 2100).unwrap();
...
fn test_e2e_stress_many_proposals() {
    ...
    for i in 0..50 {
        ...
        if i % 3 == 0 {
            ...
            // Reject others
            proposal.add_vote("gold-1".to_string(), VoteChoice::No, 100, 2000).unwrap();
            proposal.add_vote("gold-2".to_string(), VoteChoice::Yes, 100, 2100).unwrap();
        }

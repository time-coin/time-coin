...
impl StartMasternodeMessage {
    ...
    pub fn validate(&self) -> Result<(), StartProtocolError> {
        ...
        if self.alias.is_empty() {
            return Err(StartProtocolError::CryptoError("Empty alias".to_string()));
        }
...
impl CollateralVerifier {
    ...
    pub fn verify_start_message(
        ...
        if message.collateral_txid != collateral.txid || message.collateral_vout != collateral.vout
        {
...
mod tests {
    ...
    fn test_start_message_creation() {
        let message = StartMasternodeMessage::new(
            "mn1".to_string(),

...
impl MasternodeRegistry {
    ...
    pub fn withdraw_collateral(&mut self, id: &str, timestamp: u64) -> Result<u64, String> {
        let masternode = self.masternodes.get(id).ok_or("Masternode not found")?;

        if masternode.is_active() {
            return Err(
                "Cannot withdraw collateral from active masternode. Decommission first."
                    .to_string(),
            );
        }
...
mod tests {
    ...
    fn test_withdraw_collateral_after_cooldown() {
        ...
        let can_withdraw = registry
            .can_withdraw_collateral(&id, 2000 + DEFAULT_LOCK_PERIOD - 1)
            .unwrap();
        assert!(!can_withdraw);

        // Can withdraw after cooldown
        let can_withdraw = registry
            .can_withdraw_collateral(&id, 2000 + DEFAULT_LOCK_PERIOD)
            .unwrap();
        assert!(can_withdraw);

        // Withdraw collateral
        let amount = registry
            .withdraw_collateral(&id, 2000 + DEFAULT_LOCK_PERIOD)
            .unwrap();

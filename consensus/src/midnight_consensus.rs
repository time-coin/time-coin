...
//! 5. If transaction mismatch: broadcast missing txs, recreate block
...
impl MidnightConsensusOrchestrator {
    ...
    pub async fn run_midnight_consensus(
        ...
        if am_leader {
            ...
            self.leader_create_and_propose(height, previous_hash.clone(), mempool, create_coinbase_fn).await?;
        } else {
        ...
        match self.consensus.validate_proposal(&proposal).await {
            Ok(()) => {
                ...
                self.consensus
                    .vote(height, proposal.block_hash.clone(), self.my_ip.clone(), true, None)
                    .await?;
            }
            Err(missing) => {
                println!("⚠️  Missing {} transaction(s)", missing.len());
                
                // Broadcast that we're missing transactions
                let reason = format!("missing_tx:{}", missing.join(","));
                self.consensus
                    .vote(height, proposal.block_hash.clone(), self.my_ip.clone(), false, Some(reason))
                    .await?;
                
                // Request missing transactions from peers
                println!("📡 Requesting missing transactions...");
                // TODO: Implement network layer to request transactions
                
                return Err("Missing transactions - waiting for sync".to_string());
            }
        }
        ...
        let (has_consensus, approvals, required, rejections) = 
            self.wait_for_consensus(height, &proposal.block_hash, 30).await;
        
        if has_consensus {
            println!("✅ CONSENSUS REACHED ({}/{})", approvals, required);
            
            if !rejections.is_empty() {
                println!("   Note: {} rejection(s) but reached threshold", rejections.len());
    ...
    async fn leader_create_and_propose(
        ...
        println!("   Coinbase + {} mempool transactions", transactions.len() - 1);
    ...
    async fn wait_for_proposal(&self, height: u64) -> Result<BlockProposal, String> {
        for i in 0..300 {  // 30 seconds (100ms intervals)

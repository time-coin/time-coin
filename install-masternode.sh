...
show_summary() {
    ...
    echo "  time-cli wallet create-mnemonic       # Generate new 12-word recovery phrase"
    echo "  time-cli wallet import-mnemonic       # Import from existing recovery phrase"
    echo "  time-cli wallet balance <ADDRESS>     # Check address balance"
    echo "  time-cli wallet send <FROM> <TO> <AMOUNT>  # Send TIME coins"
    echo ""
    echo "  Note: Addresses start with TIME0 (testnet) or TIME1 (mainnet)"
    echo "────────────────────────────────────────────────"
    echo ""
    ...
    echo "────────────────────────────────────────────────"
    echo ""
    echo -e "${BLUE}Address Format Information:${NC}"
    echo "────────────────────────────────────────────────"
    echo "TIME Coin uses BIP-39 mnemonic phrases and hierarchical"
    echo "deterministic (HD) wallets for address generation."
    echo ""
    ...
    echo "  TIME0abc123def456...  (testnet)"
    echo "  TIME1xyz789ghi012...  (mainnet)"
    echo ""
    echo "Generate wallet addresses using:"
    echo "  • time-cli wallet commands"
    echo "  • wallet-gui application (recommended)"
    echo "────────────────────────────────────────────────"
    echo ""
    echo -e "${BLUE}Next Steps:${NC}"
    echo "────────────────────────────────────────────────"
    echo "1. Configure Masternode Address (if collateralized):"
    echo "   Edit $CONFIG_DIR/testnet.toml"
    echo "   Set: masternode.address = \"TIME0your_address_here\""
    echo "   (Testnet addresses start with TIME0)"
    echo ""
    echo "2. Restart the service after config changes:"
    echo "   sudo systemctl restart ${SERVICE_NAME}"
    echo ""
    echo "3. Monitor the node:"
    echo "   sudo journalctl -u ${SERVICE_NAME} -f"
    echo "────────────────────────────────────────────────"
    echo ""

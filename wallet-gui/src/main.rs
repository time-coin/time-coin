    ...
    fn show_main_screen(&mut self, ctx: &egui::Context) {
        ...
        egui::TopBottomPanel::top("nav_bar").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if ui.selectable_label(self.current_screen == Screen::Overview, "🏠 Overview").clicked() {
                    self.current_screen = Screen::Overview;
                }
                if ui.selectable_label(self.current_screen == Screen::Send, "📤 Send").clicked() {
                    self.current_screen = Screen::Send;
                }
                if ui.selectable_label(self.current_screen == Screen::Receive, "📥 Receive").clicked() {
        ...
        if let Some(manager) = &self.wallet_manager {
            ...
            ui.horizontal(|ui| {
                ...
                ui.vertical(|ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            ui.label(egui::RichText::new(format!("{} TIME", Self::format_amount(balance))).strong());
                        });
                    });
                    ui.separator();
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            ui.label(egui::RichText::new(format!("{} TIME", Self::format_amount(balance))).strong().size(16.0));
                        });
                    });
                });
                ...
                ui.vertical(|ui| {
                    ...
                    ui.label(egui::RichText::new("No transactions yet").color(egui::Color32::GRAY).italics());
                    ui.add_space(10.0);
    ...
    fn show_send_screen(&mut self, ui: &mut egui::Ui, _ctx: &egui::Context) {
        ...
        if let Some(_manager) = &self.wallet_manager {
            ...
            if ui.button("Send").clicked() {
                self.success_message = Some("Transaction sent! (Network integration pending)".to_string());
            }
        }

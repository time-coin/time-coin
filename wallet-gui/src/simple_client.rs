...
impl SimpleClient {
    ...
    pub async fn get_transactions(&self, xpub: &str) -> Result<Vec<TransactionRecord>> {
        ...
        match response {
            NetworkMessage::WalletTransactionsResponse { transactions, .. } => {
                let tx_records = transactions.into_iter().map(|tx| TransactionRecord {
                    tx_hash: tx.tx_hash,
            ...
            _ => Err(ClientError::InvalidResponse("Expected WalletTransactionsResponse".into())),
        }
    }
    ...
    pub async fn register_xpub(&self, xpub: &str) -> Result<()> {
        ...
        match response {
            ...
            _ => Err(ClientError::InvalidResponse("Expected XpubRegistered".into())),
        }
    }
}

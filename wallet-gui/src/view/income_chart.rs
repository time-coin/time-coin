...
fn render_income_chart(ui: &mut Ui, state: &AppState) {
    // Income = block rewards only.  Any transaction tagged "Block Reward" is a
    // masternode reward and should be counted; everything else (sends,
    // consolidations, receives from external transfers) is excluded.
    // This is far more reliable than trying to filter out non-income txs,
    // which breaks whenever consolidation receives are misclassified by the node.
    let approved_txs: Vec<_> = state
        .transactions
        .iter()
        .filter(|tx| {
            tx.memo.as_deref() == Some("Block Reward")

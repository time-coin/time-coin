        ...
        tokio::select! {
            ...
            Some(event) = ui_rx.recv() => {
                match event {
                    ...
                    UiEvent::SendTransaction { to, amount, fee: _display_fee, memo, from_address, payment_request_id } => {
                        if let Some(ref client) = state.client {
                            if let Some(ref mut wm) = state.wallet {
                                // Retry loop: wait for locked UTXOs to finalize
                                let max_retries = 5;
                                let mut effective_from_address = from_address.clone();
                                let mut same_address_self_send = false;
                                if let Some(ref selected_from) = effective_from_address {
                                    same_address_self_send = selected_from == &to;
                                } else if state.addresses.contains(&to) {
                                    match client.get_utxos(&to).await {
                                        Ok(utxos) => {
                                            let same_address_balance: u64 = utxos
                                                .iter()
                                        ...
                                        Err(e) => {
                                            log::warn!(
                                                "Failed to inspect self-send source address {}: {}",
                                                to,
                                                e
                                            );
                                        }
                                ...
                                let actual_fee = wallet::calculate_send_fee(amount, same_address_self_send);
                                let total_needed = amount.saturating_add(actual_fee);
                                let mut all_utxos = Vec::new();
                                ...
                                let addrs_to_query: Vec<String> = match effective_from_address.as_deref() {
                                ...
                                for attempt in 0..max_retries {
                                    ...
                                    tx_result = wm.create_transaction_with_change_address(
                                        &to,
                                        amount,
                                        actual_fee,
                                        effective_from_address.as_deref(),
                                    );
                                    if tx_result.is_ok() {
                                        break;
                                    }
...
async fn fetch_spendable_utxos_by_addr(
    client: &MasternodeClient,
    addresses: &[String],
...
async fn consolidate_utxos_background(
    ...
    let mut utxos_by_addr = fetch_spendable_utxos_by_addr(&client, &addresses).await;
    let initial_utxo_count: usize = utxos_by_addr.values().map(|v| v.len()).sum();
    if initial_utxo_count == 0 {
    ...
    let mut remaining_groups = 0usize;
    let mut remaining_utxos = 0usize;
    let mut hit_round_limit = false;
    let mut stalled = false;
    let mut round = 0usize;

    while !utxos_by_addr.is_empty() {
        if !consolidation_active.load(Ordering::Relaxed) {
        ...
        round += 1;
        let round_input_utxos: usize = utxos_by_addr.values().map(|v| v.len()).sum();
        let round_batches: usize = utxos_by_addr
            .values()
            .map(|utxos| utxos.len().div_ceil(BATCH_SIZE))
            .sum();
        let round_groups = utxos_by_addr.len();
        ...
        for (dest_addr, addr_utxos) in &utxos_by_addr {
            ...
            for chunk in addr_utxos.chunks(BATCH_SIZE) {
                ...
                let _ = svc_tx.send(ServiceEvent::ConsolidationProgress {
                    batch: batch_idx,
                    total_batches: batch_idx + round_batches.saturating_sub(1),
                    message: format!(
                        "Round {}: consolidating {} UTXOs on {}…",
                        round,
                        chunk.len(),
                    ...
                    Ok(bytes) => {
                        ...
                        match client.broadcast_transaction(&tx_hex).await {
                            Ok(txid) => {
                                ...
                                let _ = svc_tx
                                    .send(ServiceEvent::TransactionInserted(sent_record.clone()));
                                if let Some(ref db) = wallet_db {
                                    let _ = db.save_send_record(&sent_record);
                                }
                            }
        ...
        if remaining_utxos >= round_input_utxos {
            stalled = true;
            log::warn!(
                "Consolidation stalled after round {}: {} spendable UTXOs across {} addresses remain",
                round,
                remaining_utxos,
                remaining_groups
            );
        ...
        if round >= MAX_ROUNDS {
            ...
            log::warn!(
                "Consolidation hit round limit after {} rounds: {} spendable UTXOs across {} addresses remain",
                round,
                remaining_utxos,
                remaining_groups
            );
        ...
        let _ = svc_tx.send(ServiceEvent::ConsolidationProgress {
            ...
            total_batches: batch_idx,
            message: format!(
                "Round {} complete: {} address(es) still need consolidation.",
                round,
                round_groups.min(remaining_groups)
            ),
        });
    }
    ...
    let msg = if stalled {
        format!(
            "Consolidation stalled after {} batch(es): {} address(es) still have {} spendable UTXOs.",
            consolidated,
            remaining_groups,
            remaining_utxos
        )
        ...
        format!(
            "Consolidation paused after {} rounds: {} address(es) still have {} spendable UTXOs.",
            round, remaining_groups, remaining_utxos
        )
    } else if failed == 0 {

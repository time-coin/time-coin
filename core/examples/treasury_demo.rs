...
fn main() {
    ...
    let db_path = env::temp_dir()
        .join(format!("treasury_demo_{}", std::time::SystemTime::now()
    ...
    let mut state = BlockchainState::new(genesis.clone(), &db_path)
    ...
    let counts = MasternodeCounts { free: 1, bronze: 0, silver: 0, gold: 0 };
    ...
    println!("   Balance: {} (expected: {})", treasury_balance, expected_allocation);
    println!("   Allocations: {}\n", state.treasury().allocations().len());

    let proposal_amount = treasury_balance / 2;
    state.approve_treasury_proposal("proposal-1".to_string(), proposal_amount)
        .expect("Failed to approve proposal");
    println!("✅ Proposal approved for: {}\n", proposal_amount);

    state.distribute_treasury_funds(
    ...
    println!("   Treasury allocated: {}", chain_stats.treasury_total_allocated);
    println!("   Treasury distributed: {}\n", chain_stats.treasury_total_distributed);
